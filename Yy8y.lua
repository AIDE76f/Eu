...
-- الإعدادات الرئيسية (نفس الهيكل ولكن مع التأكد من عدم وجود أخطاء)
local Settings = {
    ESP = {
        Enabled = false,
...
-- دوال الرسم (كما هي)
local function CreateESP(Player) -- كما في السابق
...
local function UpdateESP(Player, Drawings) -- كما في السابق
...
local function BuildUI()
    ...
    -- دوال مساعدة لإنشاء عناصر داخل الـ ScrollingFrame
    local function CreateToggle(parent, text, settingPath, yVar)
        ...
        btn.ZIndex = 101
        btn.Parent = parent
        btn.Position = UDim2.new(0, 10, 0, yVar)

        local ind = Instance.new("Frame")
        ind.Size = UDim2.new(0, 20, 0, 20)
        ind.Position = UDim2.new(1, -25, 0, 5)
        ind.BackgroundColor3 = (settingPath[1] and settingPath[1][settingPath[2]] or settingPath[3]) and Color3.new(0,1,0) or Color3.new(1,0,0)
        ...
        btn.MouseButton1Click:Connect(function()
            if settingPath[1] then
                settingPath[1][settingPath[2]] = not settingPath[1][settingPath[2]]
                TweenService:Create(ind, TweenInfo.new(0.2), {BackgroundColor3 = settingPath[1][settingPath[2]] and Color3.new(0,1,0) or Color3.new(1,0,0)}):Play()
            else
                settingPath[3] = not settingPath[3]
                TweenService:Create(ind, TweenInfo.new(0.2), {BackgroundColor3 = settingPath[3] and Color3.new(0,1,0) or Color3.new(1,0,0)}):Play()
            end
        end)
    ...
    local function CreateInput(parent, labelText, settingPath, yVar, minVal, maxVal)
        ...
        lbl.ZIndex = 101
        lbl.Parent = parent
        lbl.Position = UDim2.new(0, 10, 0, yVar)
        yVar = yVar + 20
        ...
        box.Text = tostring(settingPath[1][settingPath[2]])
        box.Font = Enum.Font.GothamBold
        box.TextSize = 14
        box.ZIndex = 101
        box.Parent = parent
        box.Position = UDim2.new(0, 10, 0, yVar)
    ...
    local function CreateDropdown(parent, text, settingPath, options, yVar)
        local btn = Instance.new("TextButton")
        btn.Size = UDim2.new(0, 180, 0, 40)
        btn.BackgroundColor3 = Color3.new(0.15, 0.15, 0.15)
        btn.TextColor3 = Color3.new(1, 1, 1)
        btn.Text = text .. ": " .. settingPath[1][settingPath[2]]
        btn.Font = Enum.Font.GothamBold
        btn.TextSize = 14
        btn.ZIndex = 101
        btn.Parent = parent
        ...
        for i, opt in ipairs(options) do
            if opt == settingPath[1][settingPath[2]] then
                idx = i
                break
            end
        end
        ...
        btn.MouseButton1Click:Connect(function()
            idx = idx % #options + 1
            settingPath[1][settingPath[2]] = options[idx]
            btn.Text = text .. ": " .. settingPath[1][settingPath[2]]
        end)
    ...
    do
        local y = 10
        y = CreateToggle(ESPContent, "ESP", {Settings.ESP, "Enabled"}, y)
        y = CreateToggle(ESPContent, "Snapline", {Settings.ESP, "SnaplineEnabled"}, y)
        y = CreateDropdown(ESPContent, "Position", {Settings.ESP, "SnaplinePosition"}, {"Center", "Top", "Bottom"}, y)
        y = CreateToggle(ESPContent, "Rainbow", {Settings.ESP, "RainbowEnabled"}, y)
        y = CreateToggle(ESPContent, "Name ESP", {Settings.ESP, "NameESP"}, y)
        y = CreateToggle(ESPContent, "Team Check", {Settings.ESP, "TeamCheck"}, y)
        y = CreateToggle(ESPContent, "Off-Screen Arrow", {Settings.ESP, "OffScreenArrow"}, y)
    end

    -- تعبئة تبويبة Aimbot
    do
        local y = 10
        y = CreateToggle(AimbotContent, "Aimbot", {Settings.Aimbot, "Enabled"}, y)
        y = CreateToggle(AimbotContent, "FOV Circle", {Settings.Aimbot, "ShowFOV"}, y)
        y = CreateInput(AimbotContent, "FOV:", {Settings.Aimbot, "FOV"}, y, 1, 360)
        y = CreateInput(AimbotContent, "Max Distance:", {Settings.Aimbot, "MaxDistance"}, y, 1, 10000)
        y = CreateInput(AimbotContent, "Smooth Aim (1-100):", {Settings.Aimbot, "SmoothAim"}, y, 1, 100)
        y = CreateToggle(AimbotContent, "Magic Aim", {Settings.Aimbot, "MagicAim"}, y)
        y = CreateToggle(AimbotContent, "Auto Fire", {Settings.Aimbot, "AutoFire"}, y)
        y = CreateToggle(AimbotContent, "Full Head", {Settings.Aimbot, "FullHead"}, y)
    end

    -- تعبئة تبويبة Combo
    do
        local y = 10
        y = CreateToggle(ComboContent, "Infinite Jump", {Settings.Combo.InfiniteJump, "Enabled"}, y)
        y = CreateToggle(ComboContent, "Speed", {Settings.Combo.Speed, "Enabled"}, y)
        y = CreateInput(ComboContent, "Speed Value (1-6):", {Settings.Combo.Speed, "Value"}, y, 1, 6)
        y = CreateToggle(ComboContent, "No Wall", {{}, nil, Settings.Combo.NoWall}, y) -- خاصية NoWall ليست ضمن جدول فرعي
    end
